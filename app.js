...
window.addEventListener("beforeinstallprompt", e=>{
  ...
  setTimeout(()=>{
    ...
    installHint.innerText="Install for faster access & offline use";
  },3000);
});

installBtn.addEventListener("click", async ()=>{
  if(!deferredPrompt){
    installHint.innerText="Tap ⋮ menu → Add to Home screen";
    return;
  }
...
function compassDirection(d){
  const a=["North","NE","East","SE","South","SW","West","NW"];
  return a[Math.round(d/45)%8];
}
...
async function loadWeather(lat,lon,name,country){
  ...
  aqiAdvice.innerText=`Health impact: ${ai.a}`;
